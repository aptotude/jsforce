...
// Stub Logger for development
var logger = {
  info: () => {}
};

if (process.env.NODE_ENV === 'production' && process.env.LOGENTRIES_TOKEN ) {
  logger = new Logger({ token: process.env.LOGENTRIES_TOKEN });
}

// DEFAULTS FOR RETRY LOGIC
var TIMEOUT = parseInt(process.env.HTTP_TIMEOUT_THRESHOLD, 10) || 15000;
var FULL_RETRIES = parseInt(process.env.HTTP_RETRIES) || 2;
var RETRIES_ENABLED = process.env.ENABLE_RETRIES ? JSON.parse(process.env.ENABLE_RETRIES) : false;
...
Transport.prototype.httpRequest = function(params, callback) {
  ...
  var createRequest = function makeRequest() {
    if (!req) {
      var opts = {
        ...params,
        maxAttempts: 3,
      ...
      if (opts.method === 'GET' && RETRIES_ENABLED) {
        var operation = retry.operation({
          retries: FULL_RETRIES
        });
        ...
        operation.attempt(function(currentAttempt) {
          ...
          req = httpRequest(opts, function(err, response) {
            if (response && response.timingPhases) {
              logger.info({
                dnsAttempts: response.attempts,
                responseTime: response.timingPhases.total,
            ...
            if (err) {
              if (err.message !== 'Aborted') {
                // Reject non-cancelled request errors
                deferred.reject(err);
              } else {
                if (operation.retry(err)) {
                  // Retry if we have available retries
                  return;
                } else {
                  // Reject if we're out of retries
                  deferred.reject(new Error('Failed to retrieve data. Please try again.'));
                }
              }
            } else {
              timedOut = false;
