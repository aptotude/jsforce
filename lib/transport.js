...
const REQUEST_TIMEOUT = parseInt(process.env.JSFORCE_HTTP_TIMEOUT, 10) || 2000;
const REQUEST_MAXATTEMPTS = parseInt(process.env.JSFORCE_HTTP_MAXATTEMPTS, 10) || 3;
const REQUEST_RETRYDELAY = parseInt(process.env.JSFORCE_HTTP_RETRYDELAY, 10) || 3500;
const REQUEST_RETRYPOSTS = process.env.JSFORCE_HTTP_RETRYPOSTS ? JSON.parse(process.env.JSFORCE_HTTP_RETRYPOSTS) : true;

const bigLogger = require('le_node');
const logger = process.env.LOGENTRIES_TOKEN ?
  new bigLogger({ token: process.env.LOGENTRIES_TOKEN }) :
  { info: () => {} };
...
Transport.prototype.httpRequest = function(params, callback) {
  ...
  var createRequest = function() {
    if (!req) {
      ...
      if (opts.method === 'POST' && !REQUEST_RETRYPOSTS) {
        opts.maxAttempts = 1;
      }
      req = httpRequest(opts, function(err, response) {
        if (response.attempts && response.attempts > 1) {
          logger.info({
            ...
            message: `Making attempt number ${response.attempts} when making HTTP ${opts.method} request to URL ${opts.url}`,
            method: opts.method,
            url: opts.url
          });
        }
